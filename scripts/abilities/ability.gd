...
var cooldown: float
## Says if the Ability is on cooldown.
var is_cooldown: bool = false
## Timer used to time the Ability cooldown.
...
## Emitted when casted by the Character.
signal casted()
## Emitted when the Ability finishes casting. Every specific Ability has to emit this at some point,
## but when that happens varies depending on the Ability.
signal finished_casting()
## Emitted when the Ability cooldown starts.
...
## Checks if the Ability is not on cooldown, if the Character can cast and if the Ability
## is not already being cast. The cooldown timer then starts and the Ability is performed.
func cast() -> void:
...
## Performs what the Ability is supposed to do. This function must be implemented
## by each specific Ability.
...
## Changes the Character the Ability belongs to. It also creates the cooldown timer.
...
## Emits the unequpping signal and ensures that the ability is set
...
## For example, the Flurry Ability needs to set its amount of projectiles remaining back to
## the default amount if it is unequipped mid-cast.
func _reset_ability() -> void:
	pass

## Resets casting and cooldown variables to their default states and
