## This autoload handles world loading and emits signals such as wave_started
## so that nodes in the scene tree can easily connect to them and detect important events.
extends Node

## The world currently loaded.
var current_world: World
## Path to the world scene, where '%d' must be replaced with an integer.
var world_path: String = "res://scenes/worlds/world_%d.tscn"
...
## Sets process mode to ALWAYS when entering the scene tree and connects to GameManager's
## loaded_main signal to ensure that the world is loaded only when Main is ready.
func _ready() -> void:
	process_mode = Node.PROCESS_MODE_ALWAYS
	GameManager.loaded_main.connect(_on_game_manager_loaded_main)

## Loads the given world and adds it as a child of the Main node. Also connects
## the wave manager's signals.
func load_world(main: Main, world_number: int) -> void:
	var world_scene_path: String = world_path % world_number
...
# Loads a world in the Main node when it is ready.
