...
## which is usually used. For example, when speed is used for movement, this value is what is
## really being used, not current_value.
var max_value_after_buffs: int
...
## Emitted when the maximum value after buffschanges. Passes the old and new values as arguments.
...
func calculate_total_buff() -> void:
	var buff_sum = 0
